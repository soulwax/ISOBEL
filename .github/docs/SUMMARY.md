...
ISOBEL supports two Docker deployment modes:

1. **Bot Only** (Dockerfile in root)
   - Single container running just the Discord bot
   - Recommended for most users
   - Minimal resource usage

2. **Bot + Web** (docker-compose.yml)
   - Three services: bot, web, auth
   - Uses Docker Compose profiles
   - Shared `/data` volume for database access

### Main Dockerfile (Bot Only)
...
**Shared Resources:**
- Network: `isobel-network` (bridge)
- Volume: `./data` mounted to all services
- Web and auth depend on bot service

**Environment Variables:**
- Bot uses: `DISCORD_TOKEN`, `SONGBIRD_BASE_URL`, `SONGBIRD_API_KEY`
- Web/Auth use: `DISCORD_CLIENT_ID`, `DISCORD_CLIENT_SECRET`, `NEXTAUTH_SECRET`, `NEXTAUTH_URL`
...
# Build with docker-compose (respects profiles)
